...
curl --verbose -k -X POST $octopus/api/releases -H "X-Octopus-ApiKey: $API_KEY" -H "Content-Type: application/json" -d '{"Version": "$RELEASE_VERSION" ,"ProjectId": "$PROJECT_ID", "ReleaseNotes":"$RELEASE_NOTES"}'
